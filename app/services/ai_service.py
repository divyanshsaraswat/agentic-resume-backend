...
class AIService:
    ...
    @staticmethod
    async def stream_chat(messages: List[Dict[str, str]], resume_content: Optional[str] = None, model: Optional[str] = None, format: str = "latex") -> AsyncGenerator[str, None]:
        """
        ...
        if resume_content:
            if format == "latex":
                ...
                system_prompt += f"\n\nCURRENT RESUME CONTENT ({format.upper()}): \n{resume_content}\n\n"
                system_prompt += f"Use the extracted text from the {format.upper()} document above as context for the user's questions."
